# THIS SCRIPT calculates scenarios of LST based on a given percentage of degraded and deforested area"
#CV<- Climate Variable
########################################################################################################
# Set study area prefix
prefix<-"IndTerr"
# Set up the working directory where all the .dbf files with the variable values are located
dir.var <- "D:/Yunuen/6PostDoc/Dashboards/LST/DBFiles"
...
df.all$AreaPr_Total<-rowSums(df.all[,c("AreaPr_0","AreaPr_1","AreaPr_2","AreaPr_3")],na.rm=TRUE)

#Calculate the 2020 true weighted average
df.all$Zero<-df.all$LST_0*df.all$AreaPr_0
df.all$One<-df.all$LST_1*df.all$AreaPr_1
df.all$Two<-df.all$LST_2*df.all$AreaPr_2
df.all$Three<-df.all$LST_3*df.all$AreaPr_3
...
# Intact forest 1, Degradation = 1.01082699, Deforestation = 1.028834311 with respect to intact forest
# New values = Intact forest 1, Degradation = 1.03516750360154, Deforestation = 1.10746275573828 with respect to intact forest
df.all$Two<-(df.all$LST_1*1.03516750360154)*df.all$AreaPr_2
df.all$Three<-(df.all$LST_1*1.10746275573828)*df.all$AreaPr_3
...
df.all$difference<-df.all$bline_wAv2020-df.all$true_wAv2020
...
for (poly in 1:length(polList)) {
  ...
  df.final$Two<-(df.all[poly,7]*1.03516750360154)*df.final$pr_deg# original proportions from deg/def scenarios
  df.final$Three<-(df.all[poly,7]*1.10746275573828)*df.final$pr_def# original proportions from deg/def scenarios
  df.final$Est_wAv<- rowSums(df.final[,c("Zero","One","Two","Three")],na.rm = TRUE)/100
  
  # Difference in celsius
  df.final$Est_difference<-df.final$Est_wAv-df.all[poly,17] ###****###
  ...
  df.final$Answer_tx<-ifelse(df.final$pr_degdef>(100-df.all[poly,11]),"Los valores superan el % de bosque intacto en 2020", ###****###
                             ifelse(df.final$Est_difference>0, paste0("+",round(df.final$Est_difference,digits=2),"°C"),
                                    ifelse(df.final$Est_difference<0, paste0(round(df.final$Est_difference,digits=2),"°C"),
                                           ifelse(NA))))
  #Symbols
  df.final$Answer_sym<-ifelse(df.final$pr_degdef>(100-df.all[poly,11])," ",###****###
                              ifelse(df.final$Est_difference>0, "+",
                                     ifelse(NA)))
  
  #In number celsius
  df.final$Answer_c<-ifelse(df.final$pr_degdef>(100-df.all[poly,11]),-999,###****###
                             ifelse(df.final$Est_difference!=-999999, round(df.final$Est_difference,digits=2)))#)))"Difference 
  ...
  df.final$ToMakeCal<-ifelse(df.final$pr_degdef>(100-df.all[poly,11]),NA,
                             ...
                             ifelse(df.final$Est_difference!=-9999999, df.final$Est_difference))#)))"Difference of ET/LST with respect to 2020"
  ...
  df.final<-df.final[,c(15,1,2,13,14,12)]
...
df.all[is.na(df.all)] <- ""
df.all<-df.all[,c(1,17)]
# Rename columns
colnames(df.all)<-c('ID','LST2020_c')

############################ Get rid of -999 ##################################

all.PolyScenarios.short<-filter(all.PolyScenarios, Answer_c!= -999)
...
#Join the df.all dataframe with "Prefix_FCareas_ET&LSTvalues.csv" generated in the 3_ET_scenariosDeg&Deg.R" scrpt
#Bring the "Prefix_FCareas_ET&LSTvalues.csv" dataset into R
etvalues <- read.table(paste0(prefix,"_FCareas_ET&LSTvalues.csv"), header = TRUE, sep = ",")
etvalues<- etvalues[,c(2:7)]
df.all<-merge(etvalues,df.all,by="ID")
